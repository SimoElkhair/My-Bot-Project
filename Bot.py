...
try:
    aliexpress = AliexpressApi(ALIEXPRESS_API_PUBLIC, ALIEXPRESS_API_SECRET,
                               models.Language.AR, models.Currency.EUR, 'zawa9')
...
btn1 = types.InlineKeyboardButton(
    "⭐️ صفحة مراجعة وجمع النقاط يوميا ⭐️", url="https://s.click.aliexpress.com/e/_onbP0wM")
btn2 = types.InlineKeyboardButton(
    "⭐️تخفيض العملات على منتجات السلة 🛒⭐️", callback_data='click')
btn3 = types.InlineKeyboardButton(
    "❤️ اشترك في القناة للمزيد من العروض ❤️", url="https://t.me/AliexpressOfersss")
btn4 = types.InlineKeyboardButton(
    "🎬 شاهد كيفية عمل البوت 🎬", url="https://t.me/AliexpressOfersss/1879")
btn5 = types.InlineKeyboardButton(
    "💰 حمل تطبيق Aliexpress عبر الضغط هنا للحصول على مكافأة 5 دولار 💰", url="https://a.aliexpress.com/_mtV0j3q")
...
def generate_coin_affiliate_link(product_id):
    ...
    try:
        ...
        print(
            f"❌ Error generating coin affiliate link for product {product_id}: {e}")
        return None
...
def generate_bundle_affiliate_link(product_id, original_link):
    ...
    try:
        ...
        print(
            f"❌ Error generating bundle affiliate link for product {product_id}: {e}")
        return None
...
def get_affiliate_links(message, message_id, link):
    try:
        ...
        if not resolved_link:
            bot.delete_message(message.chat.id, message_id)
            bot.send_message(
                message.chat.id, "❌ لم أتمكن من حل الرابط! تأكد من رابط المنتج أو أعد المحاولة.")
            return
        ...
        if not product_id:
            bot.delete_message(message.chat.id, message_id)
            bot.send_message(
                message.chat.id, "❌ لم أتمكن من استخراج معرف المنتج من الرابط.")
            return
        ...
        try:
            ...
            if product_details and len(product_details) > 0:
                ...
                print(
                    f"Product details object: {json.dumps(product_details[0].__dict__, indent=2, ensure_ascii=False)}")
                ...
                print(
                    f"Product details: {title_link}, {price_pro}, {img_link}")
                bot.delete_message(message.chat.id, message_id)
...
def get_affiliate_shopcart_link(link, message):
    try:
        ...
        affiliate_link = aliexpress.get_affiliate_links(shopcart_link)[
            0].promotion_link
...
# ... (كل الكود الخاص بك السابق يبقى كما هو)

# إعداد خادم Flask بسيط لخداع نظام Render ومنعه من إغلاق البوت
app = Flask(__name__)
...
def run():
    # Render يمرر البورت تلقائياً عبر متغير بيئة اسمه PORT
    # إذا لم يجده (تشغيل محلي) سيستخدم 8080
